	...
	<div>
        ...
        border: 1px solid #dadada;box-shadow: 0 0 10px #e8e5e5;">
                          ...
                          props.row.jobHuntingInfo.salaryMax
										...
										<li class="wanna-title__item" style="float: left;;font-size: 14px; width: 50%;
                                          color: #666; line-height: 24px;">
											...
											<template v-for="(item, index) in props.row.jobHuntingInfo.expectCity">
												<span>{{ item }}</span>
												<span v-if="index !== props.row.jobHuntingInfo.expectCity.length - 1"> ,
												</span>
											</template>
							...
							<el-col :span="22">
                              ...
                              text-overflow: ellipsis; overflow: hidden; vertical-align: middle;">
											...
											<span title="学历" class="edu-ul-txt"
												style="">{{ props.row.jobHuntingInfo.education }}</span>
											...
											<template v-for="(item, index) in props.row.jobHuntingInfo.academicYear">
												...
												<template
													v-if="index !== props.row.jobHuntingInfo.academicYear.length - 1">
													~</template>
											</template>
										</li>
									</ul>
				...
				<el-table-column align="right" style="">
					...
					<template #default="scope">
						<el-button size="default" @click="handleEditState(scope.$index, scope.row.userInfo)">状态设置
						</el-button>
...
<script setup lang="ts">
	...
	function ConvertToFrontData(arr: any) {
		...
		arr.forEach((value: any) => {
			...
			if (value.jobHuntingInfo.academicYear != null)
				value.jobHuntingInfo.academicYear = value.jobHuntingInfo.academicYear.split(',')
			if (value.jobHuntingInfo.expectCity != null)
	...
	//简历查看
	const resumeView = (index: number, row: {
		jobHuntingInfo: any;userInfo: any;
	}) => {
		//初始化简历样式
