...
<script setup lang="ts">
	...
	import {
		jobList
	} from "@/store/POJOInterface/jobList";
	import {
		firmInfo
	} from "@/store/POJOInterface/firmInfo";
	...
	//转换为前端数据
	async function ConvertToFrontData(arr: any) {
		...
		arr.forEach((value: any) => {
			...
			postFileDownload('/file/downloadFile', {
				...
				console.log("imgBlob", value.firmAvatar, imgBlob);
			});
	...
	function handleEditState(index: number, row: {
		firmId: number,
		state: boolean
	}) {
	...
	async function InitialTableData() {
		...
		await ConvertToFrontData(arr)
	}
//	初始化头像图片
	async function initImag () {
		// tableData.for
	}
